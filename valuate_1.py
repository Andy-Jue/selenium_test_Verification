...
def value_test_1(driver, url, site_num):
    source_type = ['移動源 C1','固定燃燒源 C1', '工業製程 C1', '人為逸散 C1', '輸入電力 C2', '客戶和訪客運輸 C3',
    ...
    test_source = ['mobileCombustion', 'stationaryCombustion', 'directProcessEmission', 'directFugitiveEmission',
    ...
    for i, j in zip(source_type, test_source):
        if j == 'mobileCombustion' or j == 'stationaryCombustion' or j == 'directProcessEmission':
            ...
            time.sleep(2)
            calculation_process_button(driver)
            ...
            element1 = WebDriverWait(driver, 10).until(
                EC.visibility_of_element_located((By.XPATH, '//div[@class="ant-select-item-option-content"][1]')))
            element1.click()
    ...
    for key in expected_results:
        ...
        print(
            f"{key} - 計算值: {calculated_value}, 頁面呈現值: {lifecycle_value}, 是否一致: {calculated_value == lifecycle_value}")
    ...
    for key, value in co2_results.items():
        ...
        if base_key in co2_combined_emissions:
            co2_combined_emissions[base_key] = round(co2_combined_emissions[base_key] + round(float(value) / 1000, 10),
                                                     10)
        else:
            # 如果該基礎鍵不存在，則初始化
...
def sign(driver, url, site_num):
    ...
    button = parent_tr.find_element(By.XPATH,
                                    './/button[@type="button" and contains(@class, "ant-btn ant-btn-round ant-btn-link ant-btn-icon-only text-primary  border-0")]')
    # JavaScript寫法
    ...
    link.click()
def calculation_process_button(driver):
    ...
    table_scroll_container = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.XPATH, '//div[contains(@class, "ant-table-content")]'))  # 修改為正確的滾動容器
    )
    driver.execute_script("arguments[0].scrollLeft = arguments[0].scrollWidth;", table_scroll_container)
    #定位"計算結果"按鈕位置
    carbon_emissions = table.find_element(By.XPATH,'.//td[@class="ant-table-cell calculate_table"]//button[@class="ant-btn ant-btn-primary"]//span[contains(text(),"計算過程")]')
    carbon_emissions.click()
    time.sleep(2)
    calculation_process(driver)

def calculation_process(driver):
    total_carbon_emissions = 0
    row_names = {0: "CO2", 1: "CH4", 2: "N2O"}
    results = {"CO2": [], "CH4": [], "N2O": []}
    usage = None  # 單獨存儲使用量的變數
    calculated_results = {}  # 用於存儲計算結果的字典

    has_next_page = True

    while has_next_page:
        table = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, '(//div[contains(@class, "ant-table-content")])[2]')))  # 修改為正確的滾動容器
        # 定位表頭行元素
        thead_row = table.find_element(By.XPATH, './/thead/tr')
        # 找到 "燃料種類" 和 "碳排放量" 在表頭中的索引位置
        carbon_emissions_index = -1
        ...
        if carbon_emissions_index != -1:
            ...
            for tbody in tbodies:
                ...
                # 遍歷每個行元素
                for index, row in enumerate(rows):  # 使用 enumerate 獲取 row 的索引和元素
                    row_name = row_names.get(index, f"Row_{index}")
                    # 根據索引調整 td 的 XPath
                    if index == 0:
                        adjusted_td_index = carbon_emissions_index + 1  # row=0 時 td+1
                        usage_td_index = carbon_emissions_index + 2
                        usage_elements = row.find_elements(By.XPATH, f'.//td[{usage_td_index}]')
                        if usage_elements:
                            usage = float(usage_elements[0].text)
                    else:
                        adjusted_td_index = carbon_emissions_index  # 其他情況保持不變

                    # 定位燃料種類和碳排放量元素
                    carbon_emissions_elements = row.find_elements(By.XPATH, f'.//td[{adjusted_td_index}]')

                    # 獲取燃料種類和碳排放量的值
                    for carbon_emissions_element in carbon_emissions_elements:
                        carbon_emissions = float(carbon_emissions_element.text)
                        print(f"{row_name}: {carbon_emissions}")  # 輸出對應的名稱和數據
                        results[row_name].append(carbon_emissions)  # 將數據存入對應名稱的列表
                if usage is not None:
                    for gas, emissions in results.items():
                        if emissions:
                            total_emission = sum(emissions) * usage
                            calculated_results[gas] = total_emission
                for gas, total in calculated_results.items():
                    print(f"{gas} 排放係數*使用量:{total}")
                # 更新燃料種類的碳排放量總和
            try:
                next_button = WebDriverWait(driver, 10).until(EC.element_to_be_clickable(
                    (By.XPATH, "//li[@title='下一頁']//button[@class='ant-pagination-item-link']")))
